...
class AccountPayment(models.Model):
    ...
    def create_batch_payment(self):
        if self._context.get('payment_date',False):
            _logger.info(f'\n\n\n START FUNCTION \n\n\n.')
            ...
            batch = self.env['account.batch.payment'].create({
                ...
                'payment_ids': [(4, payment.id, None) for payment in self if payment.amount != 0],
           ...
           return super().create_batch_payment()
...
class HrPayslipRun(models.Model):
    ...
    is_paid = fields.Boolean(string="Payslips Paid", store=True)
...
class HrPayslip(models.Model):
    ...
    def action_register_payment(self):
        ...
        for slip in self.filtered(lambda s: s.move_id and not s.is_paid):
            ...
            payment.account_amount = payment.amount
...
#             payment.post()
        ...
        self.is_paid = True
        self.payslip_run_id.is_paid = True
        action.update({
